...
function solveMonthlyRate(principal, emi, tenure) {
  if (emi <= principal / tenure) return emi === principal / tenure ? 0 : null;
  const payment = r => {
    if (r === 0) return principal / tenure;
    const factor = Math.pow(1 + r, tenure);
    return (principal * r * factor) / (factor - 1);
  };
  let low = 0, high = 1; // up to 100% monthly
  if (payment(high) < emi) return null;
  for (let i = 0; i < 100; i++) {
    const mid = (low + high) / 2;
    if (payment(mid) < emi) low = mid; else high = mid;
  }
  return +(((low + high) / 2) * 100).toFixed(4);
}
...
router.get('/:id', authenticate, async (req, res) => {
  try {
    ...
    if (req.user.role === 'staff' && loan.company_id !== req.user.company_id) {
      return res.status(403).json({ message: 'Access denied' });
    }
...
router.post('/', authenticate, requireRole('admin', 'staff'), async (req, res) => {
  ...
  try {
    ...
    if (!customer_id || !principal_amount || !tenure_months || !start_date || (!interest_rate && !emi_amount)) {
      return res.status(400).json({ message: 'Customer, principal, tenure, start date and either monthly interest rate or EMI amount are required' });
    }
    ...
    let resolvedRate = interest_rate !== undefined && interest_rate !== null && interest_rate !== '' ? Number(interest_rate) : null;
    let resolvedEmi = emi_amount !== undefined && emi_amount !== null && emi_amount !== '' ? Number(emi_amount) : null;

    if (resolvedRate !== null) {
      if (resolvedRate < 0) return res.status(400).json({ message: 'Interest rate cannot be negative' });
      resolvedEmi = generateSchedule(Number(principal_amount), resolvedRate, Number(tenure_months), start_date).emi;
    } else {
      resolvedEmi = Number(resolvedEmi);
      if (!Number.isFinite(resolvedEmi) || resolvedEmi <= 0) return res.status(400).json({ message: 'EMI amount must be greater than 0' });
      resolvedRate = solveMonthlyRate(Number(principal_amount), resolvedEmi, Number(tenure_months));
      if (resolvedRate === null) return res.status(400).json({ message: 'The EMI amount is not valid for this principal and tenure' });
    }

    const { emi, schedule } = generateSchedule(
      Number(principal_amount), resolvedRate, Number(tenure_months), start_date
    );
    ...
    const [loanResult] = await conn.query(
      `INSERT INTO loans (company_id, customer_id, loan_type, principal_amount, interest_rate, tenure_months, emi_amount, start_date, created_by)
       VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)`,
      [req.user.company_id, customer_id, loan_type || 'Personal', principal_amount, resolvedRate, tenure_months, emi, start_date, req.user.id]
    );
...
// PUT /api/loans/:id - edit loan, company admin only. EMI schedule is regenerated.
router.put('/:id', authenticate, requireRole('admin'), async (req, res) => {
  ...
  try {
    ...
    const [custRows] = await conn.query('SELECT id, company_id FROM customers WHERE id = ?', [customer_id]);
    if (!custRows.length || custRows[0].company_id !== req.user.company_id) return res.status(403).json({ message: 'Customer is not in your company' });
    ...
    if (resolvedRate !== null) {
      ...
      resolvedRate = solveMonthlyRate(Number(principal_amount), Number(resolvedEmi), Number(tenure_months));
      if (resolvedRate === null) return res.status(400).json({ message: 'The EMI amount is not valid for this principal and tenure' });
    }
    const { emi, schedule } = generateSchedule(Number(principal_amount), resolvedRate, Number(tenure_months), start_date);

    await conn.beginTransaction();
    await conn.query(
      `UPDATE loans SET customer_id=?, loan_type=?, principal_amount=?, interest_rate=?, tenure_months=?, emi_amount=?, start_date=? WHERE id=?`,
      [customer_id, loan_type || 'Personal', principal_amount, resolvedRate, tenure_months, emi, start_date, req.params.id]
    );
    await conn.query('DELETE FROM emis WHERE loan_id = ?', [req.params.id]);
    const values = schedule.map(row => [req.params.id, row.emi_number, row.due_date, row.emi_amount, row.principal_component, row.interest_component]);
    await conn.query(
      `INSERT INTO emis (loan_id, emi_number, due_date, emi_amount, principal_component, interest_component) VALUES ?`,
      [values]
    );
    await conn.commit();
    res.json({ message: 'Loan updated', emi_amount: emi, interest_rate: resolvedRate });
  } catch (err) {
    await conn.rollback();
...
// DELETE /api/loans/:id - company admin only
