...
async function loadLoans() {
  ...
  body.innerHTML = filtered.map(l => `
    <tr>
      ...
      <td>
        <a class="btn btn-ghost btn-sm" href="loan-details.html?id=${l.id}">View schedule</a>
        ${user.role === 'admin' ? `<button class="btn btn-ghost btn-sm" style="color:var(--bad)" onclick="deleteLoan(${l.id})">Delete</button>` : ''}
      </td>
    </tr>
  `).join('');
}
...
document.getElementById('addBtn').addEventListener('click', async () => {
  ...
  ['l_principal','l_emi','l_tenure'].forEach(id => document.getElementById(id).value = '');
...
document.getElementById('loanSave').addEventListener('click', async () => {
  ...
  if (!body.customer_id || !body.principal_amount || !body.emi_amount || !body.tenure_months || !body.start_date) {
